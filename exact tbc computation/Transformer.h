...
struct Transformer
{
	...
	void doTransform(bool isStrict)
	{
		...
		cout << "tran node size:" << tran_nodes.size() << endl;
		cout << "tran edge size:" << tran_edge_size << endl;
	}
	...
	void doCompress()
	{
		...
		cout << "compress node size:" << cmp_nodes.size() << endl;
		cout << "compress edge size:" << cmp_edge_size << endl;
		for(int i = 0;i < cmp_nodes.size();i++)
		{
			if (cmp_nodes[i].timestamps.size() != 1) 
			{
				sort(cmp_nodes[i].timestamps.begin(), cmp_nodes[i].timestamps.end(), [](int t1, int t2) { return t1 < t2; });
			}
		}
	}
};